...
vec3 pointLight(vec3 position, vec3 color){
   float tHold = 5.0f;
   float distance = length(position - fPosition ); 	
   if(distance > tHold)
	return vec3(0.0f);
   return (texture(diffuseTexture, fTexCoords).rgb * min(((tHold* 0.1f)/(distance*distance)),1.0f) * 0.3f + texture(diffuseTexture, fTexCoords).rgb * ((tHold-distance)/tHold ) * 0.1f) * intensity * color;
}
...
void main() 
{
	...
	fColor = vec4(color, 1.0f) + vec4(pointLight(bec, vec3(1.0f,0.0f,0.0f)), 1.0f) + vec4(pointLight(bec2, vec3(0.0f,1.0f,1.0f)), 1.0f) + vec4(pointLight(bec3, vec3(0.0f,0.0f,1.0f)), 1.0f);
